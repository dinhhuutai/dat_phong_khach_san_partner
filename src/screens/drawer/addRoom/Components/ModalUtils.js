...
const ModalUtils = ({setModalSelectUtils, setUtils, utils}) => {
  ...
  const getData = async () => {
    ...
    const utilsT = utils.map(util => util._id);

    const dataCheckBox = res.data.map(util => {
      return {
        ...util,
        check: utilsT.includes(util._id),
      };
    });
  ...
  const handleValueUtil = (value, index) => {
    ...
    setSelectedUtils(dataId);
  };

  const handleSave = async () => {
    setUtils(selectedUtils);
    setModalSelectUtils(false);
  };
